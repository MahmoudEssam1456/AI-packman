...
def uniformCostSearch(problem):
  ...
  fringe = util.PriorityQueueWithFunction(cost) #define the fringe as a Stack. You can check util.py
  fringe.push([(problem.getStartState(), "Stop", 0)]) # adding the first node into the fringe
  #please note that each node is represented using the path from the starting node to the it
  while not fringe.isEmpty():
      # print ("fringe: ", fringe.heap)
...
def manhattanHeuristic(position):
    currentlist=position[len(position)-1]
    posX=currentlist[0][0]
    posY=currentlist[0][1]
    cost=currentlist[2]
    cost+=abs(posX-1)+abs(posY-1)
    return cost

def euclideanHeuristic(position):
    currentlist=position[len(position)-1]
    posX=currentlist[0][0]
    posY=currentlist[0][1]
    cost=currentlist[2]
    cost+=sqrt(abs(posX-1)**2+abs(posY-1)**2)
    return cost



def aStarSearch(problem, heuristic=manhattanHeuristic): #try euclideanHeuristic
  ...
  explored = []
  fringe = util.PriorityQueueWithFunction(heuristic) #define the fringe as a Stack. You can check util.py
  fringe.push([(problem.getStartState(), "Stop", 0)]) # adding the first node into the fringe
  #please note that each node is represented using the path from the starting node to the it
  while not fringe.isEmpty():
      # print ("fringe: ", fringe.heap)
